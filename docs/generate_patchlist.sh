...
patch_dir=`mktemp -u --tmpdir -d balena_engine_generate_patchlist.XXXXX`
trap '{ rm -rf ${patch_dir}; }' EXIT

write_patch() {
	patch_name=`git log --format="%s" -1 ${1} | tr ' /:.><()"' '_'`
	echo "`printf '%04d' ${patch_num}`-${patch_name}"
	git log --format="%b" -1 --patch ${commit} > ${patch_dir}/`printf %04d ${patch_num}-${patch_name}`
}
...
manual_filters() {
	case ${1} in
		...
		4032b6778df39f53fda0e6e54f0256c9a3b1d618S) ;;
